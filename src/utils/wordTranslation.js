...
function createPopup(text, targetLang) {
	...
	// 目标语言列表
	const tgtLangOptions = constantLanguages
    ...
    <div class="ignoreClass translate-popup" style=" position:absolute;left: -800px;top: -300px;z-index:99999;width: 500px;">
      ...
      <div class="pop-header">
        <span class="pop-title">${chrome.i18n.getMessage('translateSelection')}</span>
      ...
      <div class="pop-lang-row">
        <div class="lang-box">
          <label>${chrome.i18n.getMessage('sourceLanguage')}</label>
          <select id="srcLang">
      ...
      <!-- 翻译按钮 -->
      <button id="doTrans" class="pop-btn">${chrome.i18n.getMessage('translate')}</button>
      <!-- 结果区 -->
...
function translateFunc(text, src, tgt) {
	try {
		...
		popupDiv._shadow.querySelector('#transResult').textContent =
			chrome.i18n.getMessage('translateFailed') + '：' + error.message
	}
}
